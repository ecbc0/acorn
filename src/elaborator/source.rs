...
pub enum SourceType {
    ...
    /// A goal that must be proven for a block (e.g., constraint blocks, function satisfy goals).
    /// These are distinguished from internal claims that are just part of the proof.
    /// The depth should be the subenvironment's depth (the block's interior), not the
    /// surrounding environment's depth.
    BlockGoal,
}
...
impl Source {
    ...
    pub fn block_goal(module: ModuleId, range: Range, depth: u32) -> Source {
        Source {
            module_id: module,
            range,
            source_type: SourceType::BlockGoal,
            importable: false,
            depth,
        }
    }
    ...
    pub fn description(&self) -> String {
        match &self.source_type {
            ...
            SourceType::BlockGoal => format!("block goal at line {}", self.user_visible_line()),
        }
    }
    ...
    pub fn name(&self) -> Option<String> {
        match &self.source_type {
            ...
            SourceType::BlockGoal => Some(self.user_visible_line().to_string()),
        }
    }
