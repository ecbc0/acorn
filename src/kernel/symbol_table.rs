...
mod tests {
    ...
    /// Test that type arguments in term_from_monomorph are represented as free variables,
    /// not as ground type constants. This is required for extensionality to work correctly
    /// with polymorphic constants in no_mono_symbols mode.
    #[test]
    #[cfg(feature = "no_mono_symbols")]
    fn test_type_args_are_free_variables() {
        let mut type_store = TypeStore::new();
        let mut symbol_table = SymbolTable::new();

        // Create an Arbitrary type to represent a type parameter
        let type_param = TypeParam {
            name: "T".to_string(),
            typeclass: None,
        };
        let arbitrary_type = AcornType::Arbitrary(type_param.clone());

        // Register the Arbitrary type
        type_store.add_type(&arbitrary_type);

        // Create a polymorphic constant name
        let const_name = ConstantName::unqualified(ModuleId(0), "foo");

        // Add the constant with a polymorphic type (Pi(Type, Type -> Type) for simplicity)
        let poly_type = Term::pi(Term::type_sort(), Term::type_sort());
        symbol_table.add_constant(const_name.clone(), NewConstantType::Global, poly_type);

        // Create a ConstantInstance with the Arbitrary type as a parameter
        let instance = ConstantInstance {
            name: const_name,
            params: vec![arbitrary_type.clone()],
            instance_type: AcornType::Bool, // Doesn't matter for this test
            generic_type: AcornType::Bool,
            type_param_names: vec!["T".to_string()],
        };

        // Convert to Term
        let term = symbol_table
            .term_from_monomorph(&instance, &type_store)
            .expect("term_from_monomorph should succeed");

        // The term should be an application: foo(T) where T is a type argument
        assert!(
            term.as_ref().is_application(),
        ...
        // The type argument should be a FreeVariable, not a Symbol::Type
        // This is the key assertion that catches the bug
        assert!(
            type_arg.atomic_variable().is_some(),
            "Type argument should be a FreeVariable for extensionality to work, \
             but got {:?}. Type parameters in polymorphic clauses must be \
             free variables, not ground type constants.",
            type_arg
        );
    }
}
