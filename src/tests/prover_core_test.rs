...
fn test_useful_fact_extraction() {
    ...
    processor.prover.as_ref().unwrap().get_useful_source_names(&mut name_set, &processor.normalizer);
...
fn test_concrete_proof_with_active_resolution() {
    ...
    let c = prove(&mut p, "main", "goal");
    assert_eq!(c.proof.unwrap(), vec!["not g(y) or not f(y) or h(y)"]);
}
...
fn test_concrete_proof_activating_rewrite_pattern() {
    ...
    let c = prove(&mut p, "main", "goal");
    assert_eq!(c.proof.unwrap(), vec!["g(y) = f(y)"]);
}
