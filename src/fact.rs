...
impl<'a> fmt::Display for FactRef<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            FactRef::Fact(fact) => fact.fmt(f),
            FactRef::Proposition(p) => write!(f, "prop: {}", p.source.description()),
        }
    }
}
...
impl<'a> FactRef<'a> {
    pub fn proposition(value: AcornValue, source: Source) -> Fact {
        Fact::proposition(value, source)
    }

    pub fn source(&self) -> &Source {
        match self {
            FactRef::Fact(fact) => fact.source(),
            FactRef::Proposition(p) => &p.source,
        }
    }

    pub fn used_in_normalization(&self) -> bool {
        match self {
            FactRef::Fact(fact) => fact.used_in_normalization(),
            FactRef::Proposition(_) => false,
        }
    }

    pub fn as_instance_alias(&self) -> Option<(ConstantInstance, &ConstantName, AcornType)> {
        match self {
            FactRef::Fact(fact) => fact.as_instance_alias(),
            FactRef::Proposition(_) => None,
        }
    }
}
