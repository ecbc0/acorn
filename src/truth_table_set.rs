...
impl TruthTableSet {
    ...
    pub fn insert(&mut self, clause: &Clause) {
        ...
        let polarity_sets = self.tables
            .entry(positive_clause)
            .or_insert_with(Vec::new);
        
        // Only insert if this polarity pattern doesn't already exist
        if !polarity_sets.contains(&polarities) {
            polarity_sets.push(polarities);
        }
    }
...
mod tests {
    ...
    fn test_no_duplicates() {
        ...
        assert_eq!(polarity_sets.len(), 1, "Should not have duplicate polarity patterns");
        ...
        for (_, polarity_sets) in &set.tables {
            ...
            for i in 0..polarity_sets.len() {
                for j in i+1..polarity_sets.len() {
                    assert_ne!(polarity_sets[i], polarity_sets[j], 
    ...
    fn test_same_literals_different_polarities() {
        ...
        let total_patterns: usize = set.tables.values()
            .map(|v| v.len())
            .sum();
        assert_eq!(total_patterns, 4, "Should have exactly 4 unique polarity patterns");
    }
}